...
let rec cp (e:expr) : expr =
  match e with
  | Int _ | Bool _ | Unit -> e
  | Binop (op, e1, e2) -> Binop (op, cp e1, cp e2)
  | If (b, t, e) -> If (cp b, cp t, cp e)
  | Var _ -> e
  | Let (x, e1, e2) -> Let (x, cp e1, cp e2)
  | Pair (e1, e2) -> Pair (cp e1, cp e2)
  | Fst e -> Fst (cp e)
  | Snd e -> Snd (cp e)
  | Match (_e1, _x, _y, _e2) ->
      failwith "Not implemented"
  | IsPair e -> IsPair (cp e)
  | Fun (x, body) -> Fun (x, cp body)
  | Funrec (f, x, body) -> Funrec (f, x, cp body)
