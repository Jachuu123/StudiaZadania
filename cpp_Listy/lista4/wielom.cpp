...
double& wielom::operator[](int i) {
    if (i < 0 || i > n)
        throw out_of_range("Indeks spoza zakresu.");
    // Jeżeli zmieniamy najwyższy współczynnik przy wielomianie o stopniu > 0 na 0, zgłaszamy wyjątek.
    if (i == n && n > 0 && a[i] == 0.0)
        throw invalid_argument("Współczynnik przy najwyższej potędze nie może być 0.");
    return a[i];
}
...
wielom& wielom::operator*=(const wielom &v) {
    ...
    // Jeżeli najwyższy współczynnik jest zerowy (dla wielomianu niezerowego) – zgłaszamy wyjątek.
...
// Format wejścia: najpierw liczba całkowita określająca stopień,
// a następnie kolejne współczynniki (od a[0] do a[stopnia]).
...
// Wypisujemy w formacie: a_n x^n + a_(n-1) x^(n-1) + ... + a_1 x + a_0
